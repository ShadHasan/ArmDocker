...
def callback_impl(result, userdata, state):
    ...
    if state == LLMCallState.RKLLM_RUN_FINISH:
        ...
        print("run error")
        sys.stdout.flush()
    elif state in (LLMCallState.RKLLM_RUN_NORMAL, LLMCallState.RKLLM_RUN_WAITING) or result:
        try:
            global_state = state
...
class RKLLM(object):
    def __init__(self, model_path, lora_model_path = None, prompt_cache_path = None, platform = "rk3588"):
        ...
        ret = self.rkllm_init(ctypes.byref(self.handle), ctypes.byref(rkllm_param), ctypes.byref(self.callback), max_context_len=4096, max_next_len=1024)
        if (ret != 0):
            print("\nrkllm init failed\n")
            exit(0)
        else:
    ...
    def run(self, role, enable_thinking, conversation_history, sampling_params=None, max_new_tokens=None):
        rkllm_input = RKLLMInput()
        union_prompt = RKLLMInputUnion(prompt_input=ctypes.c_char_p(conversation_history.encode('utf-8')))
        ...
        # rkllm_input._union_data = union_prompt
        rkllm_input.prompt_input = ctypes.c_char_p(conversation_history.encode('utf-8'))
        ...
        self.rkllm_run(self.handle, ctypes.byref(rkllm_input), ctypes.byref(self.rkllm_infer_params), ctypes.byref(self.callback))
...
def run_inference(rkllm_model, prompt, role="user", enable_thinking=False, sampling_params=None, max_new_tokens=None):
    ...
    while not model_thread_finished or global_state in (-1, LLMCallState.RKLLM_RUN_NORMAL, LLMCallState.RKLLM_RUN_WAITING):
...
if __name__ == "__main__":
    ...
    # Fix frequency
    command = "sudo ./fix_freq_rkllm.sh"
    ...
    print("=========init....===========")
    sys.stdout.flush()
    rkllm_model = RKLLM("/mnt/nvme01/rkllm_model/qwen3_5_4b.rkllm", "", "", "rk3588")
    ...
    try:
        while True:
            ...
            # later these last message. since here we are using single message. Last message has no meaning. 
            # It useful for web intigration. since they will continously chat with append.
            data = {
            ...
            output = run_inference(rkllm_model, str(prompt).strip(), role, enable_thinking, sampling_params, max_tokens)
            print(output)
            # 4. CRUCIAL: Append the completed AI response to the history for the NEXT turn
            conversation_history += f"{output}<|im_end|>\n"
